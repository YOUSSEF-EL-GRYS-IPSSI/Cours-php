    ...
    if (!empty($email) && !empty($password) && !empty($pseudo)){
        if (filter_var($email, FILTER_VALIDATE_EMAIL) ) {
            ...
            if (!empty($userConnected)) {
                ...
                $res->msg = "L'utilisateur " . $userConnected['name'] . " est connecte ";
                $res->userName = $userConnected['name'];
                $res->userFirstname = $userConnected['firstname'];
                $res->userPseudo = $userConnected['pseudo'];
                $res->userEmail = $userConnected['email'];
                $res->userConnect = "userConnect";
                $res->userID = $userConnected["id"];
                $res->userColor = $userConnected["color"];
